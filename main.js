...
setInterval(() => {
  if (engine.select !== null) {
    document.getElementById(engine.select).style.border = "5px solid blue"
  } else {
    if (engine.oldSelect !== null) {
      document.getElementById(engine.oldSelect).style.border = "0px solid blue"
    }
  }
...
function select(name) {
  ...
  if (name == engine.select) {
    ...
    if (engine.oldSelect !== null) {
      document.getElementById(engine.select).style.border = "0px solid blue";
      document.getElementById(engine.oldSelect).style.border = "0px solid blue";
    }
  }
}
...
function saveData() {
  ...
  for (var i = 0; i < engine.elementMakeCount; i++) {
    ...
    if (i !== engine.elementMakeCount) {
      engine.dataSave += `{ "name":"`+child.id+`", "position":["`+parseInt(child.style.left)+`","`+parseInt(child.style.top)+`","`+child.style.zIndex+`"], "scale":"`+child.style.scale+`", "src":"`+child.src+`" }`;
    } 
    if (i == engine.elementMakeCount - 1) {
      engine.dataSave += `]}`
      HPS_saveFile(engine.projectName+".HTP",engine.dataSave);
      
      console.log(JSON.parse(engine.dataSave));
    } else {
      engine.dataSave += ",                             ";
    }
  }
